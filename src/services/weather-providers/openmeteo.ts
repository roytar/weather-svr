...
import type { WeatherRangeOptions } from "../weatherService.js";
import type { WeatherData } from "../../models/weather.js";
 ...
 */
const convertPrecipValue = (unitSystem: string, value: number): number =>
...
export async function fetchWeatherData(
  ...
  if (adjustedStartDate || adjustedEndDate) {
    ...
    ).start_date = adjustedStartDate ?? todayDate;
    (
      params as typeof params & { start_date: string; end_date: string }
    ).end_date = adjustedEndDate ?? todayDate;
  ...
  const weatherData: WeatherData = {
    current: {
      ...
      precipitation: convertPrecipValue(
        unitSystem,
