 ...
 */
export function weatherRoutes(log: FastifyBaseLogger) {
  const weatherService = new WeatherService(log);

  return async function registerWeatherRoutes(fastify: FastifyInstance) {
    ...
    fastify.get(
      ...
      async (request, reply) => {
        ...
        try {
          const result =
            await weatherService.getWeatherForAddress(normalizedAddress);
          const formattedOutput = weatherService.formatWeatherOutput(
            result.location,
            result.weather,
            Array.isArray(result.timezone)
              ? result.timezone[0]
              : result.timezone,
          );
          ...
          result.weather.minutely15.time.forEach((timestamp, index) => {
            ...
            rows.push({
              ...
              temperature: Math.round(
                result.weather.minutely15.temperature_2m?.[index] ?? 0,
              ),
              rain: Number(
                (result.weather.minutely15.rain?.[index] ?? 0).toFixed(3),
              ),
              visibility: Math.round(
                result.weather.minutely15.visibility?.[index] ?? 0,
              ),
              sunshine_duration: Math.round(
                result.weather.minutely15.sunshine_duration?.[index] ?? 0,
              ),
            });
          });
    ...
    fastify.get(
      ...
      async (request, reply) => {
        ...
        try {
          ...
          const { streetLine, localityLine, countryLine, coordinatesLine } =
            formatLocationDisplay(result.location);
          ...
          return reply.view("weather-day.hbs", {
            ...
            apparentTempMax: Math.round(
              result.weather.daily.apparent_temperature_max?.[
                selectedDayIndex
              ] ?? 0,
            ),
    ...
    fastify.get(
      ...
      async (request, reply) => {
        ...
        if (
          !isValidIsoDate(normalizedStartDate) ||
        ...
        if (
          requestedStartDate > maxFutureDate ||
        ...
        try {
          ...
          const now = new Date();
          const todayKey = dateKeyInTimeZone(now, timezone);
          const currentDayIndex = result.weather.daily.time.findIndex(
            (day) => dateKeyInTimeZone(day, timezone) === todayKey,
          );
          ...
          const isCurrentDaytime =
            headerSunrise && headerSunset
              ? now >= headerSunrise && now <= headerSunset
              : true;

          const headerIcon = wmoToOpenWeatherIcon(
            result.weather.current.weather_code,
            isCurrentDaytime,
          );
          const { streetLine, localityLine, countryLine, coordinatesLine } =
          ...
          const rangeStartLabel =
            result.weather.daily.time[0].toLocaleDateString("en-US", {
              timeZone: timezone,
              weekday: "short",
              month: "short",
              day: "numeric",
          ...
          const rangeEndLabel = result.weather.daily.time[
            result.weather.daily.time.length - 1
          ...
          const allHourlyRows = result.weather.hourly.time
            .map((hourTime, hourIndex) => ({
              ...
              hourDate: hourTime,
              windDirectionDegrees: Math.round(
                result.weather.hourly.windDirection[hourIndex],
              ),
              hour: hourTime.toLocaleTimeString("en-US", {
                timeZone: timezone,
                hour: "2-digit",
              ...
              temperature: Math.round(
                result.weather.hourly.temperature[hourIndex],
              ),
              precipitation: Number(
                result.weather.hourly.precipitation[hourIndex].toFixed(2),
              ),
              precipitationProbability: Math.round(
                result.weather.hourly.precipitation_probability[hourIndex] ?? 0,
              ),
              rain: Number(result.weather.hourly.rain[hourIndex].toFixed(2)),
              snow: Number(
                result.weather.hourly.snowfall[hourIndex].toFixed(2),
              ),
          ...
          const dailyRows = result.weather.daily.time.map((day, index) => {
            ...
            const rowIcon = wmoToOpenWeatherIcon(
            ...
            return {
              ...
              highTemp: Math.round(
                result.weather.daily.temperature_2m_max[index],
              ),
              lowTemp: Math.round(
                result.weather.daily.temperature_2m_min[index],
              ),
              apparentTempMax: Math.round(
                result.weather.daily.apparent_temperature_max?.[index] ?? 0,
              ),
              precipitationProbabilityMax: Math.round(
                result.weather.daily.precipitation_probability_max?.[index] ??
                  0,
              ),
              maxWindSpeed: Math.round(
                result.weather.daily.wind_speed_10m_max[index],
              ),
              maxGusts: Math.round(
                result.weather.daily.wind_gusts_10m_max[index],
              ),
              sunriseTime: result.weather.daily.sunrise[
                index
              ]?.toLocaleTimeString("en-US", {
              ...
              currentTime: showCurrentConditions
                ...
                : null,
              currentTemp: Math.round(result.weather.current.temperature_2m),
              currentWindSpeed: Math.round(
                result.weather.current.wind_speed_10m,
              ),
              currentWindDir: degreesToCardinal(
                result.weather.current.wind_direction_10m,
              ),
          ...
          return reply.view("weather-range.hbs", {
            ...
            highTemp: Math.round(
              result.weather.daily.temperature_2m_max[headerDayIndex],
            ),
            lowTemp: Math.round(
              result.weather.daily.temperature_2m_min[headerDayIndex],
            ),
            apparentTempMax: Math.round(
              result.weather.daily.apparent_temperature_max?.[headerDayIndex] ??
                0,
            ),
            maxWindSpeed: Math.round(
              result.weather.daily.wind_speed_10m_max[headerDayIndex],
            ),
            maxGusts: Math.round(
              result.weather.daily.wind_gusts_10m_max[headerDayIndex],
            ),
            precipitationProbabilityMax: Math.round(
              result.weather.daily.precipitation_probability_max?.[
                headerDayIndex
