...
export class WeatherService {
    ...
    async geocodeAddress(address) {
        ...
        try {
            ...
            const parsedLatLon = parsedBoundingBox?.center ?? parseLatLonInput(address);
            const inputHasStreetAddress = !parsedLatLon && !parsedBoundingBox && hasStreetAddressInput(address);
            let location;
            let reverseResults;
            if (parsedBoundingBox) {
                ...
                if (!Array.isArray(reverseResults)) {
                    throw new Error("Bounding-box reverse geocoding returned invalid response");
                }
                ...
                this.log?.info({ address, ...parsedLatLon }, "reverse geocoding coordinates");
                reverseResults = await geocoder.reverse({
                    lat: parsedLatLon.latitude,
                    lon: parsedLatLon.longitude,
                });
                if (!Array.isArray(reverseResults)) {
                    throw new Error("Coordinate reverse geocoding returned invalid response");
                }
                location = reverseResults[0];
                if (!location?.latitude || !location?.longitude) {
                    this.log?.warn({
                ...
                if (!location?.latitude || !location?.longitude) {
                    this.log?.error({ address, locations }, "geocode lookup returned invalid coordinates");
                    throw new Error(`Geocode lookup returned invalid coordinates for ${address}`);
                }
